...
from losses import photometric_loss, geometric_loss, compute_psnr, scale_regularization, opacity_regularization
...
def denormalize_gaussians(means3D: torch.Tensor, scales: torch.Tensor,
                          norm_center: torch.Tensor, norm_scale: torch.Tensor):
    """Map composed Gaussian means/scales from normalized [-1,1]³ space back to
    world space, so the rasterizer (which operates on untouched world-space
    cameras) sees geometrically valid Gaussians.

    xyz_world   = xyz_norm   * norm_scale + norm_center
    scale_world = scale_norm * norm_scale
    Quaternions, opacity, and SH coefficients are scale/translation-invariant.

    If norm_center/norm_scale is None (normalization disabled), returns inputs
    unchanged — anchors are already in world space in that mode.
    """
    if norm_center is None or norm_scale is None:
        return means3D, scales
    means3D = means3D * norm_scale + norm_center
...
def train(config_path: str, resume_stage: int = 1, restart_latest: bool = False):
    ...
    # Build per-scene meta lookup (anchors + normalization constants).
    # Anchors live in normalized [-1,1]³ space; norm_center/norm_scale bridge
    # back to world space via denormalize_gaussians() before rendering.
    first_dataset = multi_dataset.scenes[0]["dataset"]
    P = first_dataset.num_patches

    scene_anchors = {}
    for scene_info in multi_dataset.scenes:
        ds = scene_info["dataset"]
        # Normalization is optional. When disabled, xyz_anchor/scale_anchor are
        # already in world space and denormalize_gaussians becomes a no-op.
        has_norm = (ds.norm_center is not None) and (ds.norm_scale is not None)
        scene_anchors[scene_info["name"]] = {
            "xyz_anchor":   ds.xyz_anchor.cuda(),                                   # [P*K, 3]
            "scale_anchor": ds.scale_anchor.cuda(),                                 # [P*K, 3]
            "norm_center":  ds.norm_center.cuda() if has_norm else None,            # [3] or None
            "norm_scale":   torch.tensor(float(ds.norm_scale), device="cuda") if has_norm else None,
        }
        print(f"  [scene_anchors] {scene_info['name']}: normalization={'on' if has_norm else 'off'}")

    # Create models — no scene-specific buffers
    ...
    if should_skip_stage1:
        ...
        s1_rgb = s1_weights.get("rgb", 1.0)
        s1_ssim_target = s1_weights.get("ssim", 0.85)
        s1_geo = s1_weights.get("geo", 0.0)
        s1_scale_reg = s1_weights.get("scale_reg", 0.0)
        s1_opacity_reg = s1_weights.get("opacity_reg", 0.0)
        # Progressive schedules — match overfit_coffee_martini Stage 1 exactly
        scale_anneal_target = cfg["training"].get("scale_anneal_target", 1.0)
        scale_anneal_start = cfg["training"].get("scale_anneal_start", 0)
        ssim_warmup_steps = cfg["training"].get("ssim_warmup_steps", 500)

        optimizer_s1 = optim.AdamW(canonical_head.parameters(), lr=s1_lr)

        # If restarting within stage 1, load optimizer state first
        ...
        # Create scheduler positioned at start_step_s1 within the full s1_steps schedule.
        # last_epoch=start_step_s1-1 so the scheduler's internal init step() lands at start_step_s1.
        # Fresh start: last_epoch=-1 (default). Restart from step N: last_epoch=N-1.
        scheduler_s1 = optim.lr_scheduler.CosineAnnealingLR(
            optimizer_s1, T_max=s1_steps, last_epoch=start_step_s1 - 1
        )
        ...
        for step in pbar:
            ...
            # SSIM warmup: start L1-dominant, ramp SSIM over first ssim_warmup_steps.
            # Matches overfit_coffee_martini/src/train.py:214.
            lambda_ssim = s1_ssim_target * min(step / max(ssim_warmup_steps, 1), 1.0)

            # Scale annealing: hold at 1.0 until scale_anneal_start, then linearly ramp
            # to scale_anneal_target by the end of stage 1. Matches baseline formula at
            # overfit_coffee_martini/src/train.py:218-223.
            if step < scale_anneal_start or scale_anneal_target >= 1.0:
                scale_factor = 1.0
            else:
                progress = min((step - scale_anneal_start) / max(s1_steps - scale_anneal_start, 1), 1.0)
                scale_factor = 1.0 - (1.0 - scale_anneal_target) * progress
            ...
            for scene_batch in scene_batches:
                ...
                meta = scene_anchors[scene_name]
                canonical = canonical_head(
                    tokens_mean,
                    xyz_anchor=meta["xyz_anchor"],
                    scale_anchor=meta["scale_anchor"],
                )

                # Compose once — canonical is frame-invariant in stage 1
                means3D, scales, rotations, opacity, shs = compose_gaussians(
                    canonical, scale_factor=scale_factor
                )

                # Option A: denormalize NN outputs into world space before render.
                # xyz_world = xyz_norm * s + c;  scale_world = scale_norm * s
                means3D, scales = denormalize_gaussians(
                    means3D, scales, meta["norm_center"], meta["norm_scale"]
                )
                ...
                for i, fidx in enumerate(scene_batch["frame_indices"]):
                    for cam_name in scene_batch["cam_names"]:
                        ...
                        scene_loss = scene_loss + s1_rgb * photometric_loss(
                            rendered, gt, lambda_ssim=lambda_ssim
                        )
                        p = compute_psnr(rendered, gt)
                        total_psnr += p
                        scene_psnr_sum += float(p)
                        total_renders += 1
                        scene_renders += 1
                    ...
                    # Geometric loss: once per frame (not per cam). points_map_frames
                    # is in world space (dataset.points_map kept unnormalized), which
                    # matches the denormalized means3D now in world space.
                    if s1_geo > 0:
                        pts_map = scene_batch["points_map_frames"][i].cuda()
                        scene_loss = scene_loss + s1_geo * geometric_loss(means3D, pts_map)
                ...
                # Regularization (frame-invariant — compute once per scene).
                # Scale reg runs on world-space scales to match baseline magnitudes.
                if s1_scale_reg > 0:
                    scene_loss = scene_loss + s1_scale_reg * scale_regularization(scales)
                if s1_opacity_reg > 0:
                    scene_loss = scene_loss + s1_opacity_reg * opacity_regularization(opacity)

                per_scene_psnr[scene_name] = scene_psnr_sum / max(scene_renders, 1)
            ...
            writer.add_scalar("stage1/lambda_ssim", lambda_ssim, step)
            writer.add_scalar("stage1/scale_factor", scale_factor, step)
            for name, p in per_scene_psnr.items():
                writer.add_scalar(f"stage1/psnr_{name}", p, step)

            if step % cfg["training"]["eval_every"] == 0:
                with torch.no_grad():
                    novel_psnrs = {}
                    for eval_scene in multi_dataset.scenes:
                        eval_name = eval_scene["name"]
                        eval_cam_name = eval_scene["eval_camera"]
                        eval_cam = eval_scene["cameras"][eval_cam_name]
                        eval_dataset = eval_scene["dataset"]

                        # Re-run canonical head for this scene (loop overwrote means3D/scales)
                        eval_meta = scene_anchors[eval_name]
                        tokens_mean_eval = eval_dataset.get_tokens_mean().cuda()
                        canonical_eval = canonical_head(
                            tokens_mean_eval,
                            xyz_anchor=eval_meta["xyz_anchor"],
                            scale_anchor=eval_meta["scale_anchor"],
                        )
                        m3d, sc, rot, op, sh = compose_gaussians(
                            canonical_eval, scale_factor=scale_factor
                        )
                        m3d, sc = denormalize_gaussians(
                            m3d, sc, eval_meta["norm_center"], eval_meta["norm_scale"]
                        )

                        gt_novel = eval_dataset.load_frame_image(eval_cam_name, 0).cuda()
                        rendered_novel, _, _, _ = render_gaussians(
                            m3d, sc, rot, op, sh, eval_cam, bg_color, sh_degree
                        )
                        novel_psnr = float(compute_psnr(rendered_novel, gt_novel))
                        novel_psnrs[eval_name] = novel_psnr
                        writer.add_scalar(f"stage1/novel_psnr_{eval_name}", novel_psnr, step)

                    mean_novel = sum(novel_psnrs.values()) / max(len(novel_psnrs), 1)
                    writer.add_scalar("stage1/novel_psnr_mean", mean_novel, step)
                    detail = ", ".join(f"{n}={v:.2f}" for n, v in novel_psnrs.items())
                    tqdm.write(f"  Step {step}: train={avg_psnr:.2f}, novel[{detail}], mean={mean_novel:.2f} dB")
    ...
    if should_skip_stage2:
        ...
        scheduler_s2 = optim.lr_scheduler.CosineAnnealingLR(
            optimizer_s2, T_max=s2_steps, last_epoch=start_step_s2 - 1
        )
        ...
        for step in pbar:
            ...
            for scene_batch in scene_batches:
                ...
                for i, fidx in enumerate(scene_batch["frame_indices"]):
                    ...
                    # Option A: denormalize into world space for the rasterizer
                    means3D, scales = denormalize_gaussians(
                        means3D, scales, meta["norm_center"], meta["norm_scale"]
                    )
            ...
            if step % cfg["training"]["eval_every"] == 0:
                with torch.no_grad():
                    ...
                    for scene_info in multi_dataset.scenes:
                        ...
                        eval_meta = scene_anchors[scene_name]
                        canonical_eval = canonical_head(
                            tokens_mean_eval,
                            xyz_anchor=eval_meta["xyz_anchor"],
                            scale_anchor=eval_meta["scale_anchor"],
    ...
    if should_skip_stage3:
        ...
        for step in pbar:
            ...
            for scene_batch in scene_batches:
                ...
                tokens_mean = scene_batch["tokens_mean"].cuda()
                meta = scene_anchors[scene_batch["scene_name"]]
                canonical = canonical_head(
                    tokens_mean,
                    xyz_anchor=meta["xyz_anchor"],
                    scale_anchor=meta["scale_anchor"],
