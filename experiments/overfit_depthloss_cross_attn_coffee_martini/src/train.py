...
from losses import photometric_loss, compute_psnr, scale_regularization, opacity_regularization, depth_loss
        ...
        for step in pbar:
            ...
            for i, fidx in enumerate(batch["frame_indices"]):
                ...
                for cam_name in batch["cam_names"]:
                    ...
                    # Depth supervision: compare rendered depth to STV2 point cloud depth
                    if s2_weights.get("depth", 0) > 0 and cam_name == cfg["data"]["input_camera"]:
                        pts_map_t = batch["points_map_frames"][i].cuda()
                        total_loss = total_loss + s2_weights["depth"] * depth_loss(depth, pts_map_t, cam)
        ...
        for step in pbar:
            ...
            for i, fidx in enumerate(batch["frame_indices"]):
                ...
                for cam_name in batch["cam_names"]:
                    ...
                    # Depth supervision
                    if s3_weights.get("depth", 0) > 0 and cam_name == cfg["data"]["input_camera"]:
                        pts_map_t = batch["points_map_frames"][i].cuda()
                        total_loss = total_loss + s3_weights["depth"] * depth_loss(depth, pts_map_t, cam)
