...
class HybridDPTCanonicalGaussianHead(nn.Module):
    ...
    def __init__(self, dim_in: int = 2048, dim_hidden: int = 512,
        ...
        self.gate = nn.Parameter(torch.tensor(0.0))  # starts at 0: DPT disabled at init
    ...
    def _init_weights(self):
        # Zero-init output_proj so DPT path contributes nothing at start
        nn.init.zeros_(self.output_proj.weight)
        nn.init.zeros_(self.output_proj.bias)
        # Gaussian param heads
    ...
    def forward(self, tokens_mean: torch.Tensor):
        ...
        x_2d = tokens_padded.view(self.grid_h, self.grid_w, -1)
        ...
        # Back to sequence: [1, dpt_dim, H, W] -> [P, dpt_dim] (slice to P)
        fused_seq = fused.squeeze(0).permute(1, 2, 0).reshape(grid_size, -1)[:P]  # [P, dpt_dim]
