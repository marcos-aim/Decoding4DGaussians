...
# Force unbuffered output so progress prints immediately when piped
...
def precompute_with_hooks(cache_dir: str, cam_name: str, num_frames: int,
    ...
    """
    print(f"\n=== Loading VGGT4Track model ===", flush=True)
    model = VGGT4Track.from_pretrained("Yuxihenry/SpatialTrackerV2_Front")
    model.eval()
    model = model.to("cuda")

    print(f"=== Loading frames from {cam_name} ===", flush=True)
    ...
    # Strip DINOv2 register tokens — the model prepends 5 non-spatial register
    # tokens to the patch sequence. Drop them so num_patches == spatial grid size.
    N_REGISTER = 5
    tokens = tokens[:, N_REGISTER:]
