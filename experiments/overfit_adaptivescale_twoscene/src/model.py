...
class CanonicalGaussianHead(nn.Module):
    """Produces canonical Gaussian params from time-averaged tokens.

    xyz_anchor and scale_anchor are passed at runtime (not stored as buffers),
    enabling multi-scene training with per-scene geometric priors.
    """
    ...
    def _init_weights(self):
        # Zero-init xyz and scale so model starts exactly at xyz_anchor / scale_anchor
    ...
    def forward(self, tokens_mean: torch.Tensor,
                xyz_anchor: torch.Tensor = None,
                scale_anchor: torch.Tensor = None):
        """
        Args:
            tokens_mean:  [P, dim_in] time-averaged STV2 tokens
            xyz_anchor:   [P*K, 3] patch centroid positions (normalized space), or None
            scale_anchor: [P*K, 3] log(nn_dist) per patch, broadcast to K and 3 axes, or None
        Returns:
            dict with keys: xyz, log_scale, logit_opacity, rot, sh, hidden
        """
        ...
        xyz = xyz_residual + xyz_anchor if xyz_anchor is not None else xyz_residual

        log_scale = self.scale_head(x).view(P * K, 3)
        if scale_anchor is not None:
            log_scale = log_scale + scale_anchor
...
def compose_gaussians(canonical: dict, deltas: dict = None, scale_factor: float = 1.0):
    ...
    if deltas is None:
        ...
        scales = scale_factor * torch.exp(canonical["log_scale"])
        rotations = canonical["rot"]
        opacity = torch.sigmoid(canonical["logit_opacity"])
        shs = canonical["sh"]
    else:
        means3D = canonical["xyz"] + deltas["dxyz"]
        scales = scale_factor * torch.exp(canonical["log_scale"] + deltas["dscale"])
