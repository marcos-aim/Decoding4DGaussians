"""Full DPT + Adaptive Scale training using shared engine infrastructure.

Merges:
- DPTCanonicalGaussianHead (4-level synthetic pyramid + full DPT fusion)
- Per-patch NN-distance scale_anchor initialization (adaptive scale)
- All proven fixes: LLFF alignment, float32 tokens, always include input_camera
"""
...
def build_heads(cfg, tokens, points_map):
    ...
    # Per-patch NN-distance adaptive scale initialization
...
def train(config_path: str, start_phase: int = 0):
    ...
    scale_anneal_target = raw_cfg.get("model", {}).get("scale_anneal_target", 1.0)
    ...
    def sample_batch():
        ...
        other_cams = [c for c in train_cams if c != input_cam_name]
        extra = random.sample(other_cams, min(supervision_cams - 1, len(other_cams)))
        cam_names = [input_cam_name] + extra
    ...
    print(f"\n{'='*60}")
    print(f"TRAINING COMPLETE — DPT Full + Adaptive Scale")
    print(f"{'='*60}")
