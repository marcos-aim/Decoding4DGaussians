...
H2 + Tier 0 defensive measures + exponential LR schedule.

LR schedule change: CosineAnnealingLR replaced with geometric (exponential) decay:
    lr(t) = lr_init * (lr_final / lr_init) ^ (t / T)
This is linear on a log scale. It front-loads high-LR exploration and back-loads
precision convergence, eliminating the plateau oscillation seen with CosineAnnealingLR.
Stage 3 starts at lr_stage2_init * 0.1 to avoid destabilizing the warm model.
"""
...
from losses import (photometric_loss, compute_psnr, scale_regularization,
                    opacity_regularization, velocity_regularization)


def make_exp_scheduler(optimizer, lr_init: float, lr_final: float, total_steps: int):
    """Geometric LR decay: lr(t) = lr_init * (lr_final/lr_init)^(t/T).

    Linear on log scale. 100x decay over total_steps is typical (lr_final = lr_init/100).
    """
    ratio = lr_final / lr_init
    def lr_lambda(step):
        return ratio ** (step / max(total_steps, 1))
...
def build_heads(cfg, dataset):
    ...
    print(f"  HybridDPTCanonicalHead: {sum(p.numel() for p in canonical_head.parameters())/1e6:.1f}M params")
...
def train(config_path: str, resume_stage: int = 1):
    ...
    # ===== STAGE 1: Hybrid DPT Canonical Head =====
    if resume_stage <= 1:
        print("\n" + "=" * 60)
        print("STAGE 1: Training Hybrid DPT+MLP Canonical Head (static scene)")
        print("=" * 60)
        ...
        lr_init = s1_cfg["lr_init"]
        lr_final = s1_cfg["lr_final"]

        optimizer_s1 = optim.AdamW(canonical_head.parameters(), lr=lr_init)
        scheduler_s1 = make_exp_scheduler(optimizer_s1, lr_init, lr_final, s1_cfg["steps"])
        torch.cuda.reset_peak_memory_stats()

        pbar = tqdm(range(s1_cfg["steps"]), desc="Stage 1 (Hybrid DPT+MLP)")
        for step in pbar:
            ...
            for cam_name in batch["cam_names"]:
                ...
                for i in range(len(batch["frame_indices"])):
                    ...
                    total_loss = total_loss + s1_weights["rgb"] * photometric_loss(
                        rendered_clamped, gt, lambda_ssim=s1_weights["ssim"], win_size=ssim_win_size
                    )
                    total_psnr += compute_psnr(rendered_clamped, gt)
                    num_renders += 1
            ...
            if step % 1000 == 0:
                with torch.no_grad():
                    ...
                    rendered_novel = rendered_novel.clamp(0, 1)
                    novel_psnr = compute_psnr(rendered_novel, gt_novel)
                    writer.add_scalar("stage1/novel_psnr", novel_psnr, step)
                    tqdm.write(f"  Step {step}: train={avg_psnr:.2f}, novel={novel_psnr:.2f} dB  lr={cur_lr:.2e}")
    ...
    if resume_stage <= 2:
        ...
        lr_init = s2_cfg["lr_init"]
        lr_final = s2_cfg["lr_final"]

        optimizer_s2 = optim.AdamW(deformation_head.parameters(), lr=lr_init)
        scheduler_s2 = make_exp_scheduler(optimizer_s2, lr_init, lr_final, s2_cfg["steps"])
        torch.cuda.reset_peak_memory_stats()
        ...
        for step in pbar:
            ...
            for i, fidx in enumerate(batch["frame_indices"]):
                ...
                for cam_name in batch["cam_names"]:
                    ...
                    total_loss = total_loss + s2_weights["rgb"] * photometric_loss(
                        rendered_clamped, gt, lambda_ssim=s2_weights["ssim"], win_size=ssim_win_size
                    )
                    total_psnr += compute_psnr(rendered_clamped, gt)
            ...
            if s2_weights.get("velocity_reg", 0) > 0:
                total_loss = total_loss + s2_weights["velocity_reg"] * velocity_regularization(deltas["dxyz"])
            ...
            cur_lr = scheduler_s2.get_last_lr()[0]
            vram_alloc, _ = get_vram_gb()
            pbar.set_postfix({"loss": f"{total_loss.item():.4f}", "psnr": f"{avg_psnr:.1f}",
                               "lr": f"{cur_lr:.2e}", "vram": f"{vram_alloc:.1f}/{total_vram:.0f}GB"})
            writer.add_scalar("stage2/loss", total_loss.item(), step)
            writer.add_scalar("stage2/psnr", avg_psnr, step)
            writer.add_scalar("stage2/lr", cur_lr, step)

            if step % cfg["training"]["checkpointing"]["render_every"] == 0:
                save_render(rendered_clamped, os.path.join(render_dir, "stage2", f"step_{step:05d}.jpg"))
            if step % cfg["training"]["checkpointing"]["save_every"] == 0 and step > 0:
    ...
    if "stage3" in cfg["training"]:
        ...
        lr_init = s3_cfg["lr_init"]
        lr_final = s3_cfg["lr_final"]

        optimizer_s3 = optim.AdamW(
            list(canonical_head.parameters()) + list(deformation_head.parameters()),
            lr=lr_init,
        )
        scheduler_s3 = make_exp_scheduler(optimizer_s3, lr_init, lr_final, s3_cfg["steps"])
        torch.cuda.reset_peak_memory_stats()
        ...
        for step in pbar:
            ...
            for i, fidx in enumerate(batch["frame_indices"]):
                ...
                for cam_name in batch["cam_names"]:
                    ...
                    total_loss = total_loss + s3_weights["rgb"] * photometric_loss(
                        rendered_clamped, gt, lambda_ssim=s3_weights["ssim"], win_size=ssim_win_size
                    )
                    total_psnr += compute_psnr(rendered_clamped, gt)
            ...
            if s3_weights.get("velocity_reg", 0) > 0:
                total_loss = total_loss + s3_weights["velocity_reg"] * velocity_regularization(deltas["dxyz"])
            ...
            cur_lr = scheduler_s3.get_last_lr()[0]
            vram_alloc, _ = get_vram_gb()
            pbar.set_postfix({"loss": f"{total_loss.item():.4f}", "psnr": f"{avg_psnr:.1f}",
                               "lr": f"{cur_lr:.2e}", "vram": f"{vram_alloc:.1f}/{total_vram:.0f}GB"})
            writer.add_scalar("stage3/loss", total_loss.item(), step)
            writer.add_scalar("stage3/psnr", avg_psnr, step)
            writer.add_scalar("stage3/lr", cur_lr, step)

            if step % cfg["training"]["checkpointing"]["render_every"] == 0:
                save_render(rendered_clamped, os.path.join(render_dir, "stage3", f"step_{step:05d}.jpg"))
            if step % cfg["training"]["checkpointing"]["save_every"] == 0 and step > 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str,
                        default="D:/DecodeGaussians/experiments/overfit_dpt_hybrid_explr_coffee_martini/config.yaml")
