...
def coverage_loss(gaussian_means: torch.Tensor,
                  points_map: torch.Tensor,
                  max_pts: int = 8192) -> torch.Tensor:
    """
    For each STV2 point, distance to nearest Gaussian mean.
    Encourages Gaussians to cover the full observed point cloud.
    ...
    min_dists, _ = dists.min(dim=0)  # per-point nearest Gaussian (coverage direction)
