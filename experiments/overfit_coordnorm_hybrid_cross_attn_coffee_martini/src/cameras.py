...
class MiniCam:
    ...
    @classmethod
    def from_matrices(cls, width, height, fovx, fovy, znear, zfar,
                      world_view_transform, projection_matrix):
        """Create MiniCam from pre-computed matrices (for coordinate normalization)."""
        ...
        cam.zfar = zfar
        cam.time = 0.0
        cam.world_view_transform = world_view_transform.cuda().float()
        cam.projection_matrix = projection_matrix.cuda().float()
