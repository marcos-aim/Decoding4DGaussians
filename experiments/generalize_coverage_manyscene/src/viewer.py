...
def load_model(config_path: str, scene_name: str, checkpoint_path: str = None):
    ...
    # Create dataset without coordinate normalization (matching single-scene pattern)
    ...
    if checkpoint_path is None:
        ...
        checkpoint_path = os.path.join(ckpt_dir, "stage1_step2000.pt")

    print(f"Loading checkpoint: {checkpoint_path}")
    ckpt = torch.load(checkpoint_path, weights_only=False)
    canonical_head.load_state_dict(ckpt["canonical_head"])
    canonical_head.xyz_anchor.copy_(init_xyz_per_gaussian.cuda())
