...
H2: Hybrid DPT+MLP canonical head with 2-level lightweight DPT spatial fusion
and proven per-patch MLP parameter prediction.

Tier 0 defensive changes applied:
  1. Rendered output clamped to [0,1] before loss and PSNR — prevents unbounded
     rasterizer values from inflating gradients.
  2. Velocity regularization on dxyz in stages 2/3 — dead-zone enforcement so
     deformations must earn their displacement.
  3. Pre-rasterization opacity culling in renderer.py (see that file).
  4. Scale init already resolution-aware via res_scale = REF_W / W (unchanged).
"""
...
def build_heads(cfg, dataset):
    ...
    # Resolution-aware hyperparameters (Tier 0: scale init tied to 1/sqrt(H*W))
    REF_W = 512
    res_scale = REF_W / W
    ...
    # Hybrid DPT canonical head config
...
def train(config_path: str, resume_stage: int = 1):
        ...
        for step in pbar:
            ...
            for cam_name in batch["cam_names"]:
                ...
                for i in range(len(batch["frame_indices"])):
                    ...
                    # Tier 0: clamp before loss to prevent unbounded gradient inflation
    ...
    if resume_stage <= 2:
        ...
        for step in pbar:
            ...
            # Tier 0: velocity regularization dead-zone
    ...
    if "stage3" in cfg["training"]:
        ...
        for step in pbar:
            ...
            # Tier 0: velocity regularization dead-zone (lighter in stage 3)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str,
                        default="D:/DecodeGaussians/experiments/overfit_tier0_defensive_coffee_martini/config.yaml")
