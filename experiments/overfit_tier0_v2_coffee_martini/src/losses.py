...
Tier 0 change: added velocity_regularization for deformation dead-zone enforcement.
"""
...
def photometric_loss(rendered: torch.Tensor, target: torch.Tensor,
                     lambda_ssim: float = 0.85, win_size: int = 11) -> torch.Tensor:
    """
    ...
    ssim_val = compute_ssim(rendered, target, data_range=1.0, size_average=True, win_size=win_size)
...
def velocity_regularization(dxyz: torch.Tensor) -> torch.Tensor:
    """L2 penalty on deformation displacements — dead-zone enforcement.

    Discourages the deformation head from making unnecessary positional
    drift. Small weight (0.001) is enough to keep deformations purposeful
    without preventing legitimate motion.
    """
    return dxyz.pow(2).mean()
