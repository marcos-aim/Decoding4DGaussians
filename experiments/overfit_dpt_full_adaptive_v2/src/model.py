"""Gaussian decoder heads: Full DPT Canonical (with adaptive scale) + Cross-Attention Deformation.

Merges:
- DPTCanonicalGaussianHead (4-level synthetic pyramid + DPT fusion, from dpt_full experiment)
- Per-patch NN-distance scale_anchor initialization (from adaptivescale experiment)

The CrossAttentionDeformationHead and compose_gaussians are unchanged from baseline.
"""
...
from dpt_modules import SyntheticPyramid, DPTFusion, ImageInjector
...
class DPTCanonicalGaussianHead(nn.Module):
    """Full DPT canonical head with adaptive per-patch scale initialization.

    Architecture:
    1. Reshape tokens to 2D spatial grid
    2. 4-level synthetic pyramid (SyntheticPyramid)
    3. Top-down DPT fusion (DPTFusion)
    4. Optional RGB image injection
    5. Conv-based Gaussian parameter prediction
    6. Per-patch NN-distance scale_anchor (adaptive scale init)
    """
    ...
    def __init__(self, dim_in: int = 2048, grid_h: int = 28, grid_w: int = 38,
                 ...
                 use_image_injection: bool = False,
        ...
        # 4. Per-patch feature refinement
        hidden_dim = 256
        ...
        # Adaptive scale anchor: per-patch NN distance initialization
    ...
    def _init_weights(self):
        ...
        # Scale: zero conv output, let scale_anchor provide init; fallback to init_log_scale
    ...
    def forward(self, tokens_mean: torch.Tensor, image: torch.Tensor = None):
        ...
        # Pad/reshape to 2D spatial grid: [P, C] -> [1, C, H, W]
        pad_len = grid_size - P
        if pad_len > 0:
            tokens_padded = torch.cat([tokens_mean, tokens_mean[-1:].expand(pad_len, -1)], dim=0)
        else:
            tokens_padded = tokens_mean
        x = tokens_padded.view(self.grid_h, self.grid_w, -1).permute(2, 0, 1).unsqueeze(0)

        # Multi-scale pyramid + DPT fusion
        levels = self.pyramid(x)
        fused = self.fusion(levels)  # [1, fused_dim, grid_h, grid_w]

        # Optional image injection
        ...
        xyz_raw = self.xyz_head(features)
        scale_raw = self.scale_head(features)
        rot_raw = self.rot_head(features)
        opa_raw = self.opacity_head(features)
        sh_raw = self.sh_head(features)
        hidden = self.hidden_proj(features)  # [1, 1024, H, W]

        def _reshape(t, param_dim):
            t = t.squeeze(0).permute(1, 2, 0)            # [H, W, K*param_dim]
            t = t.reshape(grid_size, K * param_dim)[:P]  # [P, K*param_dim]
...
class CrossAttentionDeformationHead(nn.Module):
    ...
    def forward(self, canonical_hidden: torch.Tensor, frame_tokens: torch.Tensor):
        ...
        Q = self.q_proj(canonical_hidden).unsqueeze(0)
        KV = self.kv_proj(frame_tokens).unsqueeze(0)
