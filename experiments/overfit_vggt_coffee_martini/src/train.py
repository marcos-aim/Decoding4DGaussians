"""DPT Hybrid + Cross-Attention training using shared engine infrastructure.

Identical model architecture to overfit_dpt_hybrid_cross_attn_coffee_martini,
but uses the shared engine for: AMP, torch.compile, phase system, optimized
losses, async metrics, and checkpoint management.
"""
...
# Add experiment src to path for local imports
sys.path.insert(0, os.path.dirname(__file__))

# Project root for shared engine imports
...
def build_heads(cfg, tokens, points_map):
    """Build model heads from config and precomputed data."""
    model_cfg = cfg.get("model", {})
    can_cfg = model_cfg.get("canonical", {})
    defo_cfg = model_cfg.get("deformation", {})

    P = tokens.shape[1]  # [T, P, D]
    ...
    grid_h = can_cfg.get("grid_h", 28)
    grid_w = can_cfg.get("grid_w", 38)

    # Build per-Gaussian xyz initialization from point cloud
    patch_h = H / grid_h
    patch_w = W / grid_w
...
def align_points_to_llff(points_map: torch.Tensor, cache_dir: str, cameras: dict,
    ...
    # Compute scale: match STV2 near depth to LLFF near bound
...
def train(config_path: str, start_phase: int = 0):
    # Load both old-format config (for model/data) and new engine config
    ...
    # ---- Data loading (shared SceneDataset for tokens/points) ----
    ...
    # Align points from VGGT space to LLFF world space (same as old dataset._align_points_to_llff)
    ...
    points_map_cpu = points_map  # keep CPU copy for init (already aligned, CPU)
    points_map = points_map.to(device)

    # GT image loading helper
    from src.dataset import SceneDataset
    gt_loader = SceneDataset.__new__(SceneDataset)
    gt_loader.device = device
    ...
    # torch.compile on heads (NOT on CUDA rasterizer)
    if use_compile:
        print("[engine] Compiling model heads with torch.compile...")
        modules = compile_modules(
            modules, compile_enabled=True,
    ...
    # Supervision camera list (exclude eval camera)
    ...
    # ---- Pre-cache all GT frames into CPU pinned memory ----
    import cv2
    import numpy as np
    import random

    print("[engine] Pre-caching GT frames into CPU pinned memory...")
    gt_cache = {}  # gt_cache[cam_name][frame_idx] = [3, H, W] float32 pinned CPU tensor
    ...
    input_cam = engine_config.data.input_camera or "cam01"

    def sample_batch():
        frame_indices = [random.randint(0, num_frames - 1) for _ in range(batch_frames)]
        # Always include input_camera — its spatial layout matches the token grid
    ...
    # ================================================================
    #  PHASE LOOP — using shared engine's phase system
    # ================================================================
    ...
    for phase_idx, phase in enumerate(engine_config.phases):
        ...
        # Load checkpoint from previous phase if resuming
        ...
        for step in pbar:
            ...
            # ---- Model forward with AMP on heads only ----
            if use_amp:
            ...
            if is_canonical_only:
                ...
                # Deformation phases: canonical may be frozen
            ...
            # Async-friendly metrics (no .item() in hot path)
            ...
            if step % eval_every == 0:
                with torch.no_grad():
                    ...
                    # Use autocast for eval too to match training dtype expectations
    ...
    print(f"\n{'='*60}")
    print(f"TRAINING COMPLETE — New Engine (AMP={use_amp}, Compile={use_compile})")
    print(f"{'='*60}")
    ...
    print(f"  Steps/second (overall): {sum(p.steps for p in engine_config.phases) / total_elapsed:.1f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start_phase", type=int, default=0,
                        help="Phase index to start from (skip earlier phases)")
